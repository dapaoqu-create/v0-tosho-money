...
export function AiChatDialog({ open, onOpenChange }: AiChatDialogProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const assistantMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: "assistant",
        content: `抱歉，發生錯誤：${errorMsg}。請稍後再試。`,
      }
      setMessages((prev) => [...prev, assistantMessage])
    } finally {
      setIsLoading(false)
  ...
  const aiStrings = {
    assistant: "AI 助手",
    description: "協助管理系統功能",
    clear: "清除",
    welcome: "你好！我是 AI 助手，可以幫你查詢交易、對賬狀態、統計資料等。請問有什麼需要幫助的？",
    suggestions: "你可以試試以下問題：",
    suggestion1: "查詢系統統計",
    suggestion2: "搜尋確認碼",
    suggestion3: "顯示 CSV 清單",
    suggestion4: "2025年收入多少",
    thinking: "思考中...",
    placeholder: "輸入訊息...",
  }
    ...
    <div className="fixed bottom-24 right-6 z-50 w-[400px] max-w-[calc(100vw-48px)] rounded-xl border border-border bg-background shadow-2xl">
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-semibold">{aiStrings.assistant}</h3>
            <p className="text-xs text-muted-foreground">{aiStrings.description}</p>
          </div>
        </div>
      ...
      <div ref={scrollRef} className="h-[400px] overflow-y-auto p-4">
        {messages.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center text-muted-foreground">
            ...
            <p className="text-sm">{aiStrings.welcome}</p>
            <div className="mt-4 space-y-2 text-xs">
              <p className="font-medium">{aiStrings.suggestions}</p>
              <div className="flex flex-wrap justify-center gap-2">
                {[aiStrings.suggestion1, aiStrings.suggestion2, aiStrings.suggestion3, aiStrings.suggestion4].map(
                  (suggestion, i) => (
                    <button
                      key={i}
...
function MessageContent({ content, toolInvocations }: { content: string; toolInvocations?: any[] }) {
  ...
  const renderToolResult = (invocation: any) => {
    ...
    // 收入統計
    if (invocation.toolName === "getIncomeStatistics") {
      return (
        <div className="mt-2 space-y-1 text-xs">
          <div className="rounded border border-border bg-background p-2">
            <div className="text-muted-foreground">
              {result.year}年{result.month !== "全年" ? `${result.month}月` : ""} 收入
            </div>
            <div className="text-lg font-bold">¥{result.totalIncome?.toLocaleString()}</div>
            <div className="text-muted-foreground">
              已對賬: ¥{result.reconciledIncome?.toLocaleString()} | 未對賬: ¥
              {result.unreconciledIncome?.toLocaleString()}
            </div>
            <div className="text-muted-foreground">共 {result.transactionCount} 筆交易</div>
          </div>
        </div>
      )
    }
